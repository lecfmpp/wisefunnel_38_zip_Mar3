...
When I receive an audio attachment (e.g., .m4a, .mp3, .wav):

1. **Transcribe automatically** using the `transcribe-audio` script in `scripts/transcribe-audio`
2. **Read the transcript** and respond to its contents directly
3. **Do not** ask the user to repeat or clarify — the transcript is the message
4. **Do not** summarize unless explicitly requested; include relevant excerpts if needed

The script uses local Whisper (offline, free) and works for common audio formats.
